...
1. Initialize random values for `b` and `m`.
2. Compute the cost function `J(b,m)`.
3. Calculate the gradients with respect to `b` and `m`.
4. Update `b` and `m` using the update equations.
5. Repeat until the cost function reaches its minimum.
